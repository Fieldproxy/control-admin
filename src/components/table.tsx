...
function CustomTable(props: PropsI) {
  ...
  const isSelected = (name: string) =>
    props && props.selected ? props.selected.indexOf(name) !== -1 : false;
  ...
  return (
    <Paper className={classes.root}>
      <TableContainer className={classes.container} style={{maxHeight: props.maxHeight || 400}}>
                    ...
                    <Checkbox
                      indeterminate={
                        ...
                        props.selected.length <
                          props.rows.filter(
                            item => !!item && props.selectKey && !!item[props.selectKey]
                          ).length
                      }
                      checked={
                        props.selected.length > 0 &&
                ...
                .map((row, i) => {
                  ...
                  return row ? (
                    <TableRow
                      hover
                      onClick={event =>
                        props.handleClick && props.selectKey
                          ? props.handleClick(row[props.selectKey])
                          : {}
                      }
                      role="checkbox"
